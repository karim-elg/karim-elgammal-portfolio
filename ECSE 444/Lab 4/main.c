...
osThreadId ReactButtonHandle;
osThreadId ReadSensorHandle;
osThreadId PrintValHandle;
...
int main(void)
{
  ...
  /* Create the thread(s) */
  /* definition and creation of ReactButton */
  osThreadDef(ReactButton, StartReactButton, osPriorityNormal, 0, 128);
  ReactButtonHandle = osThreadCreate(osThread(ReactButton), NULL);

  /* definition and creation of ReadSensor */
  osThreadDef(ReadSensor, StartReadSensor, osPriorityAboveNormal, 0, 250);
  ReadSensorHandle = osThreadCreate(osThread(ReadSensor), NULL);

  /* definition and creation of PrintVal */
  osThreadDef(PrintVal, StartPrintVal, osPriorityNormal, 0, 250);
  PrintValHandle = osThreadCreate(osThread(PrintVal), NULL);
  ...
  while (1)
  {
  	if (sensor_state == TEMP_STATE){
  		...
  		sprintf(temp_str, "\r\nTemperature: %.2f", temp);
    	HAL_UART_Transmit(&huart1,temp_str,sizeof(temp_str),10);// Sending in normal mode
  	} else if (sensor_state == ACCEL_STATE){
  		char accel_str[40];
  		BSP_ACCELERO_AccGetXYZ(accel);
  		sprintf(accel_str, "\r\nAcceleration x:%.4d, y:%.4d, z:%.4d", accel[0], accel[1], accel[2]);
  		HAL_UART_Transmit(&huart1,accel_str,sizeof(accel_str),10);// Sending in normal mode
  	}else if(sensor_state == MAGNETO_STATE){
  		char magneto_str[40];
  		BSP_MAGNETO_GetXYZ(magnet);
  		sprintf(magneto_str, "\r\nMagnometer x:%.4d, y:%.4d, z:%.4d", magnet[0], magnet[1], magnet[2]);
  		HAL_UART_Transmit(&huart1,magneto_str,sizeof(magneto_str),10);// Sending in normal mode
  	}else if(sensor_state == PRESS_STATE){
  		char press_str[20];
  		pressure = BSP_PSENSOR_ReadPressure();
  		sprintf(press_str, "\r\nPressure: %.2f", pressure);
  		HAL_UART_Transmit(&huart1,press_str,sizeof(press_str),10);// Sending in normal mode
...
/* USER CODE BEGIN Header_StartReactButton */
/**
  * @brief  Function implementing the ReactButton thread.
  * @param  argument: Not used
  * @retval None
  */
/* USER CODE END Header_StartReactButton */
void StartReactButton(void const * argument)
{
  /* USER CODE BEGIN 5 */
  /* Infinite loop */
...
/* USER CODE BEGIN Header_StartPrintVal */
/**
* @brief Function implementing the PrintVal thread.
* @param argument: Not used
* @retval None
*/
/* USER CODE END Header_StartPrintVal */
void StartPrintVal(void const * argument)
{
